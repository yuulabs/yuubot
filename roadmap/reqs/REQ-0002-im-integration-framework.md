...
# REQ-0002: IM Integration Framework

对齐 Constitution M3 (集成框架稳定起点)。

## Problem

当前只有 `echo` / `github` / `test_im` 三个 integration 实现,且 `test_im`
只是测试桩。没有真实的 IM 集成,没有验证过"外部 IM 消息进入 → Actor 处理 →
回复原路返回"的完整链路契约。本 REQ 定义稳定的 IM 集成模板,使 QQ /
Telegram(REQ-0003 / REQ-0004)能照模板快速接入。
...
External IM user sends message (via QQ or Telegram)
  ...
  → IM conversation is stateless across daemon restarts:
      ...
      (符合宪法层决策:IM 场景不必持久 Actor;持久 Actor 是 overnight
       实验场景的需求,不属于本 REQ)
```
...
4. **Response** — Actor 通过 facade 调用 `IntegrationInstance.response()`,
   原路回复。语义是即时回复,不做延迟/定时(架构宪法约束)。

模板要求:
- IM 场景走 `Conversation + 过期 expire`,无持久 Actor(宪法层决策落点)。
...
- 具体协议库选型(QQ / Telegram 协议库)= REQ-0003 / REQ-0004 的工作。
- 持久 Actor / overnight 消息保留 = 宪法 M5,不在本 sprint。
- IM 多端同步 / 消息去重 / 离线消息队列 = 非本 sprint,未来按需加。
- 容错(YNetwork 相关)REQ-CHANGE 时再议。
