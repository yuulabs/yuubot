...
> 产品宪章：yuubot 要达成什么、到什么程度算 done。
> 架构宪章（不变量 / 扩展点 / SOP）见 `apps/yuubot/design/constitution.md`。
> 本文管"造什么、造到哪停"，架构宪章管"怎么造"。
...
5. **最终使用状态** — 一位研究员对某现象感兴趣（如 LLM 训练推理的 overhead），
   ...
   - 让 yuubot overnight 运行实验（人类晚上只能睡觉）。
     ...
     实现载体（Actor / 后台任务 / 状态存储）是架构层决策，不在产品宪章约束。
