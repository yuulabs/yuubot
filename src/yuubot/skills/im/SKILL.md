...
使用 heredoc 传入消息 JSON（避免 shell 引号转义问题）:

```
ybot im send --ctx <ctx_id> << 'EOF'
<msg_json>
EOF
```

```
ybot im send --uid <user_id> << 'EOF'
<msg_json>
EOF
```

```
ybot im send --gid <group_id> << 'EOF'
<msg_json>
EOF
```
...
示例（混合消息）:
```
ybot im send --ctx 3 << 'EOF'
[{"type":"text","text":"吃饭啦！"},{"type":"at","qq":"123456"},{"type":"text","text":" 快来～"}]
EOF
```
...
### 浏览消息
`ybot im browse --msg <msg_id> --before 10 --after 10`
`ybot im browse --ctx <ctx_id> --since "2024-01-01" --until "2024-01-31" --limit 50`
...
`ybot im list members --gid <group_id>` — 群成员列表
