...
if TYPE_CHECKING:
    from yuuagents.context import DockerExecutor, OutputBuffer
    from yuuagents.flow import Flow
...
class AgentRunBuilder:
    ...
    build_subprocess_env: Callable[..., dict[str, str]]
    ...
    async def build_task_bundle(self, turn: TurnContext) -> TaskBundle:
        ...
        items = self._build_multimodal_items(text, turn)
        if len(items) == 1:
            return TaskBundle(task_text=text, user_items=[text], is_multimodal=False)
    ...
    async def build_run_context(
        ...
        *,
        ...
        prompt_spec, prompt_builder = self.build_prompt(turn.agent_name)
    ...
    async def _memory_probe_text(self, turn: TurnContext) -> str:
        ...
        for pending in turn.pending_messages:
            parts.append(await format_segments(pending.segments))
    ...
    def _merge_messages(self, turn: TurnContext) -> InboundMessage:
        ...
        raw_event = dict(turn.message.raw_event)
        if turn.pending_messages:
            raw_event["_extra_events"] = [msg.raw_event for msg in turn.pending_messages]
        return InboundMessage(
            message_id=turn.message.message_id,
            ctx_id=turn.message.ctx_id,
            chat_type=turn.message.chat_type,
            sender=turn.message.sender,
            segments=turn.message.segments,
    ...
    def _build_handoff_message(self, turn: TurnContext) -> InboundMessage:
        raw_event = dict(turn.message.raw_event)
        raw_event["message"] = [{"type": "text", "data": {"text": turn.handoff_text}}]
        raw_event["raw_message"] = turn.handoff_text
        extra_events = [turn.message.raw_event, *[msg.raw_event for msg in turn.pending_messages]]
        raw_event["_extra_events"] = extra_events
        return InboundMessage(
            message_id=turn.message.message_id,
            ctx_id=turn.message.ctx_id,
            chat_type=turn.message.chat_type,
            sender=turn.message.sender,
            segments=to_inbound_message(raw_event).segments,
            timestamp=turn.message.timestamp,
            raw_event=raw_event,
        )

    def _build_multimodal_items(self, text: str, turn: TurnContext) -> list[object]:
        ...
        for seg in self._collect_image_segments(turn):
            url = ""
            if seg.local_path:
                url = self._file_to_data_uri(seg.local_path)
            elif seg.url:
                url = seg.url
            if url:
    ...
    def _collect_image_segments(self, turn: TurnContext) -> list[ImageSegment]:
        messages = [turn.message, *turn.pending_messages]
