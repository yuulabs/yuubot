"""PTY stdout renderer for conversation and monitor display."""
...
class PtyDisplayBuffer:
    _lines: list[str] = field(factory=lambda: [""])
    _row: int = 0
    _col: int = 0
    _pending: str = ""
...
def render_pty_output(raw: str) -> str:
...
def filter_tool_output(raw: str) -> str:
    rendered = render_pty_output(raw)
    filtered = strip_ansi(_OSC_CONTROL_RE.sub("", rendered))
    return _C0_CONTROL_RE.sub("", filtered)


async def forward_pty_snapshots(stdout: TextStream, emit: Callable[[str], None]) -> None:
    buffer = PtyDisplayBuffer()
    last = ""
