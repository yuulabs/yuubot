...
def make_chat_dialogs_handler(
    chat_store: ChatStore | None,
):
    async def chat_dialogs(request: Request) -> JSONResponse:
        if chat_store is None:
            return error_response("chat store not available", status_code=503)
...
def make_chat_dialog_messages_handler(
    chat_store: ChatStore | None,
):
    async def chat_dialog_messages(request: Request) -> JSONResponse:
        ...
        try:
            ...
            if q:
                result = await chat_store.search_messages(
                    dialog_id, q, limit=limit
                )
            else:
...
def make_chat_message_by_id_handler(
    chat_store: ChatStore | None,
):
