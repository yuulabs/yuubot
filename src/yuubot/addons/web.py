"""Web addon — search, read, download.

Runs in-process. Playwright operations run in thread pool.
"""
...
class WebAddon:
    ...
    async def search(
        ...
        **_kw,
        ...
        from yuubot.skills.web.search import _check_search_quota

        allowed, remaining = _check_search_quota()
        if not allowed:
            return [text_block("错误: 本次任务搜索次数已达上限 (3/3)")]
    ...
    async def read(
        ...
        **_kw,
        ...
        from yuubot.skills.web.reader import _fetch_and_extract, _get_profile
        import asyncio
