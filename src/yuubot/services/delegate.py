...
class DelegateService:
    delegate_actor: object | None = None
    ...
    async def delegate(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if self.delegate_actor is None:
            raise YuubotServiceError("delegate actor is not configured — migration in progress")
        ...
        # Delegate to a child agent via the actor
        from yuubot.daemon.actor import HumanMessage
        handoff_text = f"你是被 {parent_name} 委派的子 agent。请完成任务后给出结果。"
        full_prompt = f"{handoff_text}\n\n任务: {prompt}"
        msg = HumanMessage(
            ctx_id=_int(payload.get("ctx_id")),
            character_name=target,
            bot_kind="master",
            content=yuullm.user(full_prompt),
            group_id=_int(payload.get("group_id")),
            sender_id=_int(payload.get("user_id")),
        )
        try:
            await self.delegate_actor.stage.mailbox.send(msg)  # type: ignore[union-attr]
            task.status = "finished"
            task.result = "delegate dispatched"
