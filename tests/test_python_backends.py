...
from yuubot.daemon.restricted_python import RestrictedPythonWorker, _execute_restricted
...
def _shiori_bootstrap_code() -> str:
    return (
        "import yb\n"
...
def test_shiori_runtime_exposes_image_description_helpers() -> None:
    from yuuagents.python_runtime import PythonRuntime, _resolve_python
    from yuubot.characters import CHARACTER_REGISTRY

    spec = CHARACTER_REGISTRY["shiori"].spec
    runtime = _resolve_python(
        PythonRuntime(imports=spec.resolved_imports(), expand_functions=spec.expand_functions),
    )

    assert runtime is not None
    docs = runtime.tool_description_suffix()
...
def test_restricted_python_wraps_agent_fn_imports_as_sync_facades() -> None:
    ...
    from yuubot.daemon.restricted_python import _init_session_state, _refresh_namespace
...
async def test_group_execute_python_uses_restricted_worker(yuubot_config) -> None:
    ...
    with mock_llm(responses), mock_recorder_api():
        result = await runner.run_direct_turn(
            inbound,
            agent_name="yuu",
            bot_kind="group",
        )
...
@pytest.mark.asyncio
async def test_restricted_worker_close_session_drops_namespace() -> None:
    from yuuagents.python_runtime import PythonRuntime, _resolve_python

    runtime = _resolve_python(PythonRuntime())
    worker = RestrictedPythonWorker()
    try:
        first = await worker.execute(session_id="s1", runtime=runtime, code="x = 123\nx")
        await worker.close_session("s1")
        second = await worker.execute(session_id="s1", runtime=runtime, code="x")
    finally:
        worker.stop()

    assert first.status == "ok"
    assert second.status == "error"
    assert any("NameError" in line for line in second.traceback)


@pytest.mark.asyncio
@pytest.mark.skip(reason="Session reuse tracking not exposed on MateRunResult; internal to ActorTestRunner._private_python_sessions")
async def test_private_master_reuses_full_python_session(yuubot_config) -> None:
    ...
    with mock_llm(), mock_recorder_api():
        first = await runner.run_direct_turn(
            inbound,
            agent_name="yuu",
            bot_kind="master",
        )
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Signal injection not supported in Mate architecture; pending re-implementation")
async def test_running_text_model_signal_image_is_text_reference(yuubot_config) -> None:
    config = msgspec.structs.replace(
        yuubot_config,
        agent_llm_refs={
            **yuubot_config.agent_llm_refs,
            "shiori": "deepseek/deepseek-chat",
        },
    )
    runner = ActorTestRunner(config=config)
...
@pytest.mark.asyncio
async def test_shiori_bootstrap_reads_workspace_via_yb(yuubot_config) -> None:
    """Shiori bootstrap code uses `import yb` + `yb.session_state()` to locate workspace.
    ...
    responses = [
        make_tool_call_response("execute_python", json.dumps({"code": _shiori_bootstrap_code()})),
        make_text_response("done"),
    ]
    ...
    assert session is not None
    tool_step = next(step for step in session.steps if isinstance(step, ToolStep))
