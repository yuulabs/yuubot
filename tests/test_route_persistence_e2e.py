...
async def test_http_gateway_routes_persist_across_restart(tmp_path: Path) -> None:
    llm = PromptConditionedProvider(rules=[(user_message_contains("hello"), reply_text("hi"))])
    app = await boot_app(tmp_path / "data", provider=llm)
    async with running_server(app) as server:
        ...
        await create_route(server, route_id="mailbox", pattern="mailbox", actor_id="amy")
        inbound = await post_inbound(server, "mailbox", "hello", conversation_id="route-persist-c1")
        assert inbound["delivered"] is True
        await wait_for_history_kind(server, "route-persist-c1", "gen_text")

    restored = await boot_app(tmp_path / "data", provider=llm)
    async with running_server(restored) as server:
        ...
        inbound = await post_inbound(server, "mailbox", "hello", conversation_id="route-persist-c2")
        assert inbound["delivered"] is True
