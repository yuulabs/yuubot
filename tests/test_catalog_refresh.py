...
async def test_refresh_catalog_removes_stale_models(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        await app.runtime.state.upsert_model_card("deepseek", ModelCard("gpt-4o"))
...
async def test_refresh_catalog_retains_actor_bound_stale_models(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        await app.runtime.state.upsert_model_card("deepseek", ModelCard("legacy-model"))
        await app.runtime.state.upsert_model_card("deepseek", ModelCard("deepseek-chat"))
