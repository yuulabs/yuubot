...
yuullm normalises the streaming differences across LLM providers (OpenAI, Anthropic, and any OpenAI-compatible API) into a uniform `AsyncIterator[StreamItem]`. It also collects `Usage` and `Cost` after the stream ends.

`YLLMClient` is stateless: you own the history list. `YuuSession` is the optional stateful layer for history management, provider fallback, and recovery events.

### Design in One Sentence

Messages and output items are typed structs; tool specs and content blocks are plain dicts — minimal abstraction, maximum interop.
...
history = [
    yuullm.tools(tools),
    yuullm.user("What's the weather in Paris?"),
]
...
while True:
    ...
    history.append(yuullm.assistant(
    ...
    for tc in tool_calls:
        ...
        history.append(yuullm.tool(tc.id, json.dumps(result)))
```
...
| `AttemptRecovery` | `failed_provider`, `next_provider`, `rollback_to`, `continuation`, `reason` | Session-only control event emitted when a provider attempt is retried |
...
#### `client.stream(history, *, model=None, on_raw_chunk=None, **kwargs)`

Returns `(AsyncIterator[StreamItem], store)`. Tool specs belong in history via `yuullm.tools([...])`, so copying history copies the full model-visible prompt prefix. After the iterator is exhausted, `store.usage` is a `Usage` and `store.cost` is `Cost | None`.

### YuuSession

```python
session = yuullm.YuuSession(pool, selector="deepseek")
session.append(yuullm.user("Hello"))
stream, store = await session.stream()
```

`YuuSession.stream()` does not accept `model=` overrides; the selector is resolved through `ProviderPool`. On retryable provider failures it yields `AttemptRecovery`, whose `rollback_to` cursor tells callers which emitted stream position remains valid and whose `continuation` says whether the next provider can continue seamlessly.
