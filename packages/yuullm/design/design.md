...
yuullm 的低层 `YLLMClient` 不负责埋点，也不维护历史。可选的 `YuuSession` 层负责 history 管理、provider fallback，以及通过 `AttemptRecovery` 把内部重试和 rollback checkpoint 通知调用方。
...
ToolSpecs = list[dict]            # provider-neutral function tool schema
History = list[Message | ToolSpecs]
```
...
工具定义使用 `list[dict]` -- 原始 json_schema 字典。yuullm 不定义自己的 ToolSpec 类。工具定义作为 `History` item 出现，因此复制 History 会复制完整的模型可见 prompt prefix。
...
history = [yuullm.tools(tools), *messages]
...
# 便利函数
def tools(tool_specs: list[dict]) -> ToolSpecs: ...
...
# 直接作为 History item 传给 yuullm，无需任何转换
...
1. **低层无状态，可选 session** -- `YLLMClient` 不维护对话历史，history 可由上层（yuuagents）管理；需要 stateful 行为时使用 `YuuSession`，它只提交最终 canonical assistant message，并用 `AttemptRecovery` 报告 retry checkpoint。
