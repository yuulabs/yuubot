...
class PriceCalculator:
    ...
    def _prices_from_genai(self, provider: str, model: str) -> dict[str, float] | None:
        """Query genai-prices, with suffix stripping and relay fallback."""
        try:
            __import__("genai_prices")
        except ImportError:
            return None
    ...
    def estimate(
        ...
        *prices* is a dict with keys ``input_mtok``, ``output_mtok``,
        ...
        cache_read_cost = (
            cache_read_tokens * prices.get("cache_read_mtok", 0) / 1_000_000
        )
        cache_write_cost = (
            cache_write_tokens * prices.get("cache_write_mtok", 0) / 1_000_000
        )
