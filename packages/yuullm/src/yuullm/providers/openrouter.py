...
class OpenRouterProvider(OpenAIChatCompletionProvider):
    ...
    timeout : float | None
        Request timeout in seconds. Forwarded to the underlying
        ``OpenAIChatCompletionProvider`` client.  ``None`` uses the SDK default.
    max_retries : int | None
        Maximum number of retries. Forwarded to the underlying
        ``OpenAIChatCompletionProvider`` client.  ``None`` uses the SDK default (2).
    """
    ...
    async def stream(
        ...
        **kwargs,
        ...
        lowered_history: History = []
        if tools:
            lowered_history.append(ToolSpecs(tools))
        lowered_history.extend(messages)
        return await super().stream(
            lowered_history, model=model, on_raw_chunk=on_raw_chunk, **kwargs
        )
