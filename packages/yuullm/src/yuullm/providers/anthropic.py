...
class AnthropicMessagesProvider:
    ...
    timeout : float | None
        Request timeout in seconds. Passed through to the underlying
        ``AsyncAnthropic`` client.  ``None`` uses the SDK default.
    max_retries : int | None
        Maximum number of retries. Passed through to the underlying
        ``AsyncAnthropic`` client.  ``None`` uses the SDK default (2).
    """
        ...
        for message in messages:
            if message.role == "user":
                ...
                result.append(
                    {
                        "role": "user",
                        "content": content_blocks(items, preserve_cache_control=True),
                        **message.provider_extra,
                    }
                )

            elif message.role == "assistant":
                content, thinking_items, tool_calls = split_assistant_items(
                    message.content
                )
                blocks: list[dict] = []
                for ti in thinking_items:
                    if is_thinking_item(ti):
                        block: dict = {"type": "thinking", "thinking": ti["thinking"]}
                        sig = ti.get("signature")
                        if sig is not None:
                            block["signature"] = sig
                        blocks.append(block)
                    elif is_redacted_thinking_item(ti):
                        blocks.append({"type": "redacted_thinking", "data": ti["data"]})
                blocks.extend(content_blocks(content, preserve_cache_control=True))
    ...
    def _convert_tools(tools: list[dict[str, Any]]) -> list[dict]:
        ...
        for t in tools:
            ...
            if cache_control := t.get("cache_control"):
    ...
    async def stream(
        ...
        **kwargs,
        ...
        store = Store()
        messages, tools = split_history(history)
    ...
    async def _iterate(
        ...
        # Accumulate thinking blocks by block index: thinking text + signature
        thinking_acc: dict[int, dict[str, str]] = {}
        # Redacted thinking blocks: index -> data string
        redacted_acc: dict[int, str] = {}
                ...
                if event_type == "message_start":
                    ...
                    if block_type == "tool_use":
                        ...
                        if isinstance(data, str):
                            redacted_acc[current_block_index] = data
                    ...
                    if delta_type == "thinking_delta":
                        ...
                        if isinstance(thinking, str):
                            if current_block_index in thinking_acc:
                                thinking_acc[current_block_index]["thinking"] += (
                                    thinking
                                )
                            yield Reasoning(item={"type": "text", "text": thinking})
                    elif delta_type == "signature_delta":
                        sig = getattr(delta, "signature", None)
                        if isinstance(sig, str) and current_block_index in thinking_acc:
                            thinking_acc[current_block_index]["signature"] += sig
                    ...
                    tc_acc = tool_calls_acc.pop(current_block_index, None)
                    if tc_acc is not None:
                        yield ToolCall(
                            id=tc_acc["id"],
                            name=tc_acc["name"],
                            arguments=tc_acc["arguments"],
                        )
                    th_acc = thinking_acc.pop(current_block_index, None)
                    if th_acc is not None:
                        yield ThinkingBlock(
                            thinking=th_acc["thinking"],
                            signature=th_acc["signature"] or None,
                        )
                    rd_data = redacted_acc.pop(current_block_index, None)
                    if rd_data is not None:
                        yield ThinkingBlock(
                            thinking="",
                            redacted_data=rd_data,
                        )
