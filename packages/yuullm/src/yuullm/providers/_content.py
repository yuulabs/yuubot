...
def split_assistant_items(
    ...
    list[ContentItem], list[ThinkingItem | RedactedThinkingItem], list[ToolCallItem]
]:
    content: list[ContentItem] = []
    thinking: list[ThinkingItem | RedactedThinkingItem] = []
    tool_calls: list[ToolCallItem] = []
    for item in items:
        if is_tool_call_item(item):
            tool_calls.append(item)
        elif is_thinking_item(item) or is_redacted_thinking_item(item):
            thinking.append(item)  # type: ignore[arg-type]
        elif is_content_item(item):
            content.append(cast(ContentItem, item))
        else:
            raise TypeError(
                "assistant messages only accept content, thinking, and tool-call items"
            )
