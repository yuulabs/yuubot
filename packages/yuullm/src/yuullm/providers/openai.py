...
class OpenAIChatCompletionProvider:
    ...
    timeout : float | None
        ...
        ``AsyncOpenAI`` client.  ``None`` uses the SDK default.
    max_retries : int | None
        Maximum number of retries. Passed through to the underlying
        ``AsyncOpenAI`` client.  ``None`` uses the SDK default (2).
    """
    ...
    async def _iterate(
        ...
        # Accumulate all reasoning text for ThinkingBlock at end of stream
        ...
        async for chunk in response:
            ...
            if reasoning_text:
                reasoning_acc.append(reasoning_text)
        ...
        # Emit complete thinking block for round-trip (DeepSeek / OpenRouter)
        if reasoning_acc:
            yield ThinkingBlock(thinking="".join(reasoning_acc))
