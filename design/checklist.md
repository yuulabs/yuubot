...
#### Admin Conversation 与系统通道

这些能力属于yuubot自身运行时：

1. Admin Conversation工作区
2. 定时任务触发
3. Admin操作触发
4. bridge回连触发
5. 后台任务状态变化
...
Admin Conversation属于yuubot的first-party Admin workspace。它不是IM产品，也不是Integration消息入口。

- Conversation由Admin显式创建，API前缀为`/api/admin/conversations`。
- Conversation绑定`actor_id`，但交互对象是该Actor配置出的conversation-mode Agent线程。
- Conversation Agent继承Actor的character、LLM、tool/resource facade、budget和trace策略。
- Conversation拥有独立history、SSE event stream、agent lifecycle和workspace状态。
- Conversation不会进入Actor mailbox，也不会和Integration/IM通知流混在一起。
- Admin负责浏览器session认证、stream转发、interrupt/continue等工作区控制。

Conversation（`conversation_id`）是Admin工作区线程标识。历史消息和事件进入平台DB的`conversations`与`conversation_messages`，因为它们是可恢复的工作上下文，而不是临时UI状态。

Admin Conversation不是Integration，因此不提供`integration.response()`，也不使用Actor mailbox分拣。它可以直接支持Integration消息难以表达的内容，例如：
...
IM和其他Integration走正常消息入口：外部平台消息 -> Integration -> Gateway -> Actor mailbox -> Actor分拣。Actor决定是否回复、如何路由、是否委派给Agent、是否调用`yb.im.respond()`或Integration capability。

这条路径是通知流和事件流，允许多来源消息涌入同一个Actor mailbox。它和Admin Conversation的专注工作区语义不同：Admin Conversation面向一个明确的Agent线程，默认不被其他通知打扰。
...
Admin Conversation和其他系统通道不属于Integration插件系统。Admin Conversation、定时任务、Admin操作、bridge回连、后台任务状态变化等System能力不应出现在插件列表、Integration列表、Gateway source选择器或Actor的Integration启用列表中。
...
以下条目是计划中的内置Integration插件。它们提前写在yuubot代码里，但仍然属于Integration插件系统，和Admin Conversation/System通道是两类能力。
...
然后就应当可以访问Admin页面。登录之后，用户需要去LLM Provider页面配置供应商（提供预设，只需写api key）。随后，去Actor页面配置Agent(预设) + LLM models(`provider/model:effort`格式)，即可启动Actor, 紧接着Admin Conversation就将可用。
