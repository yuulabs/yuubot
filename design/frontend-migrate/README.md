...
| --- | --- | --- |
| Gateway | `GET/PUT /api/gateway`, probe, virtual model catalog | Structured Gateway status and configuration page |
| Actors | `PUT/DELETE /api/actors/{id}`, enable/disable, inbound | Structured model selector and Actor lifecycle pages |
...
- Gateway implementation boundary: `src/yuubot/llm/gateway.py`.
- Current shell nav: `web/src/features/shell/app-layout.tsx`.
- Current resource pages: `web/src/features/gateway/`, `web/src/features/actors/`, and `web/src/features/integrations/`.
...
- Preserve true bindings. Actor edit must round-trip the stored virtual `model`, `persona`, and `workspace` instead of reconstructing from snapshots.
