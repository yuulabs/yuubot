...
- **Integration 有生命周期**：Loader（代码级注册）→ 运行实例（DB 持久化，`factory.create()` 创建即激活）→ `IntegrationIngress`（运行时入口）。启用时平台只注入 `Gateway` 和私有 `IntegrationStorage.data_dir`，停用时释放 ingress 但 IntegrationConfig 与私有数据目录保留。
...
- **LLM Backend 与 Integration 完全分离**：
  ...
  - Integration 是外部连接和能力扩展（`integrations` 表），通过 capability manifest 和 `IntegrationFactoryRegistry` 接入。新增搜索、SaaS 或内部服务走 capability-based contract。
...
- **扩展点边界统一**：三方服务和新渠道按 Integration / Capability 模型接入；新增能力应主要新增一个 IntegrationFactory 和 capability manifest，而不是修改多条业务路径。IM 平台也是 Integration，只是恰好只提供 Channel。
